...
DEFAULT_CONFIG = {
    "site_title": "SAYEM PREMIUM VPS",
    "site_header": "SAYEM VPS",
    ...
    "passwords": {
        "secret": hashlib.sha256("SAYEMXADMIN24".encode()).hexdigest(),
        "user": hashlib.sha256("sayem".encode()).hexdigest()
    }
}
...
async def api_check(message: types.Message):
    ...
    try:
        ...
        r = requests.get(url, timeout=15, headers={
            'User-Agent': 'Mozilla/5.0 (SAYEM VPS Bot)'
        })
...
def change_password():
    try:
        ...
        if hashed_current == CONFIG['passwords']['secret']:
            ...
            save_config(CONFIG)
            return jsonify({'status': 'ok', 'message': 'User password updated by sayem'})
...
if __name__ == "__main__":
    ...
    print("=" * 50)
    print("SAYEM PREMIUM VPS - Starting...")
    print("=" * 50)
    print(f"Port: {port}")
    print(f"Debug: {debug}")
    ...
    print("  Secret: SAYEMXADMIN24 (Can change user password)")
    print("  User: sayem (Cannot change password)")
    print("=" * 50)
